...
private:
    ...
    void createKey()
    {
        ...
        FILE* key = fopen("key.docx", "wb");
        FILE* plaintext;
        if (!(plaintext = fopen("plaintext.docx", "rb")))
            return;
...
public:
    void Decrypt()
    {
        _plaintext.Open("plaintext_enc.docx", "wb");
        if (!_ciphertext.Open("ciphertext.docx", "rb"))
        {
            std::cout << "Cannot open the coded file. Perhaps it doesn't exist." << std::endl;
            return;
        }

        if (!_key.Open("key.docx", "rb"))
        {
    ...
    void Encrypt()
    {
        _ciphertext.Open("ciphertext.docx", "wb");
        if (!_plaintext.Open("plaintext.docx", "rb"))
        {
