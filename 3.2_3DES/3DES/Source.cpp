...
public:
    ...
    void Encrypt(int cipher_mode)
    {
        ...
        _ciphertext.GetData().resize(_plaintext.GetData().size() + CryptoPP::DES_EDE3::BLOCKSIZE);
    ...
    void Decrypt(int cipher_mode)
    {
        ...
        _plaintext.GetData().resize(_ciphertext.GetData().size() + CryptoPP::DES_EDE3::BLOCKSIZE);
...
void main()
{
    ...
    int action;
    int mode;
    std::cout << "3DES" << std::endl << "Choose mode:" << std::endl
